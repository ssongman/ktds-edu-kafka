...
첫번째 터미널에서 실습 자료를 download 하자.

4개 모두 동일한 docker container환경이므로 어느곳에서든 한곳에서만 download 받으면 된다.
...
- python  Consumer 실행한다.
  ...
  - ```sh
    ...
    $ python ~/githubrepo/ktds-edu-kafka/kafka/PythonSample/11.consumer.py  edu-topic01
    ```
...
- python producer 초당 1회 발송
  ...
  - ```sh
    ...
    # 자신의 topic 명을 아규먼트로 입력한다.
    $ python ~/githubrepo/ktds-edu-kafka/kafka/PythonSample/12.producer.py  edu-topic01
    ```
