...
- 예상결과 : partition 1,2,3 이 모두 한꺼번에 assigned 될 것이다.
  ...
  ```
  edu-topic02-cg: partitions assigned: [edu-topic02-0, edu-topic02-1, edu-topic02-2]
  ```
...
- python  Consumer 실행한다.
  ...
  - ```sh
    ...
    $ python 11.consumer.py edu-topic02 edu-topic02-cg
...
- 예상결과 : partition 1,2,3 중 하나가 제외 처리 될 것이다.
  ...
  ```
  edu-topic02-cg: partitions assigned: [edu-topic02-0, edu-topic02-1]
  ```
...
- 예상결과 : partition 1,2,3 중 두개가 제외 처리 될 것이다.
  ...
  ```
  edu-topic02-cg: partitions assigned: [edu-topic02-0]
  ```
...
- python producer 초당 1회 발송
  ...
  - ```sh
    ...
    # 자신의 topic 명을 아규먼트로 입력한다.
    # args : topic-name
    $ python 12.producer.py edu-topic02
...
edu-topic02-a   my-cluster   1            1                    True
edu-topic02     my-cluster   3            2                    True
...
# 10 회전송
$ python 21.producer-10pub.py edu-topic02-a
...
# topic partition 수정
$ kubectl -n kafka edit kafkatopic edu-topic02-a
...
# 확인
$ kubectl -n kafka get kafkatopic edu-topic02-a
NAME            CLUSTER      PARTITIONS   REPLICATION FACTOR   READY
edu-topic02-a   my-cluster   2            1                    True  # <-- 변경되었음
...
...
$ python 22.producer-10pub_key.py edu-topic02-a key1
...
# key명을 아규먼트로 입력
$ python 22.producer-10pub_key.py edu-topic02-a key2
$ python 22.producer-10pub_key.py edu-topic02-a key3
...
$ kubectl -n kafka get kafkatopic edu-topic02-a
NAME            CLUSTER      PARTITIONS   REPLICATION FACTOR   READY
edu-topic02-a   my-cluster   3            1                    True  # <-- 변경되었음
...
...
NAME            CLUSTER      PARTITIONS   REPLICATION FACTOR   READY
edu-topic02     my-cluster   3            2                    True
edu-topic02-a   my-cluster   3            1                    True
edu-topic02-b   my-cluster   3            3                    True
...
...
$ python 25.producer-spt-ack.py edu-topic02-b 50000 0
$ python 25.producer-spt-ack.py edu-topic02-b 50000 1
$ python 25.producer-spt-ack.py edu-topic02-b 50000 all


# 로그를 확인해보자.
topicName[edu-topic02-b] range_cnt[50000] ack[0] Producing...
duration time : 4.446583032608032

topicName[edu-topic02-b] range_cnt[50000] ack[1] Producing...
duration time : 4.506533145904541

topicName[edu-topic02-b] range_cnt[50000] ack[all] Producing...
...
$ python 30.producer-spt-batch-linger.py edu-topic02-b 50000 16384 3000
$ python 30.producer-spt-batch-linger.py edu-topic02-b 50000 32768 3000
$ python 30.producer-spt-batch-linger.py edu-topic02-b 50000 65536 3000


# 로그를 확인해보자.
topicName[edu-topic02-b] range_cnt[50000] batch_size[16384] linger_ms[3000] Producing...
duration time : 4.992011070251465

topicName[edu-topic02-b] range_cnt[50000] batch_size[32768] linger_ms[3000] Producing...
duration time : 4.646012544631958

topicName[edu-topic02-b] range_cnt[50000] batch_size[65536] linger_ms[3000] Producing...
...
$ python 41.consumer-offset-reset.py edu-topic02-a edu-topic02-a1-cg latest


# 로그를 확인해보자.
KafkaConsumer topicName[edu-topic02-a] group_id[edu-topic02-a1-cg] auto_offset_reset[latest]  ...
Consuming...
...
$ python 41.consumer-offset-reset.py edu-topic02-a edu-topic02-a2-cg earliest

# 로그를 확인해보자.
KafkaConsumer topicName[edu-topic02-a] group_id[edu-topic02-a2-cg] auto_offset_reset[earliest]  ...
Consuming...

...
topic=edu-topic02-a partition=1 offset=19: key=b'key3' value=b'{"eventName":"a","num":6,"title":"a", "writeId":"", "writeName": "", "writeDate":"" }'
topic=edu-topic02-a partition=1 offset=20: key=b'key3' value=b'{"eventName":"a","num":7,"title":"a", "writeId":"", "writeName": "", "writeDate":"" }'
topic=edu-topic02-a partition=1 offset=21: key=b'key3' value=b'{"eventName":"a","num":8,"title":"a", "writeId":"", "writeName": "", "writeDate":"" }'
topic=edu-topic02-a partition=1 offset=22: key=b'key3' value=b'{"eventName":"a","num":9,"title":"a", "writeId":"", "writeName": "", "writeDate":"" }'
...
...
```sh
$ python 41.consumer-offset-reset.py edu-topic02-a edu-topic02-a1-cg earliest

KafkaConsumer group_id[edu-topic02-a1-cg] auto_offset_reset[earliest]  ...
topicName[edu-topic02-a] subscribed!
Consuming...
...
edu-topic02-a1-cg 은 이미 사용했으므로 current offset 이 이미 lastest 로 저장되었다.
...
# 대량 데이터 전송
# args : topic, 발송건수
$ python 47.producer-spt-ack.py edu-topic02-b 50000

KafkaProducer...
topicName[edu-topic02-b] range_cnt[50000] Producing...
...
# args : topic, group-id, auto_offset_reset
$ python 45.consumer-poll.py edu-topic02-b edu-topic02-b-cg latest
...

TopicPartition(topic='edu-topic02-b', partition=0) 500
TopicPartition(topic='edu-topic02-b', partition=0) 500
TopicPartition(topic='edu-topic02-b', partition=0) 500
TopicPartition(topic='edu-topic02-b', partition=0) 500
TopicPartition(topic='edu-topic02-b', partition=0) 500
TopicPartition(topic='edu-topic02-b', partition=0) 249
...
...
TopicPartition(topic='edu-topic02-b', partition=2) 500
TopicPartition(topic='edu-topic02-b', partition=2) 500
TopicPartition(topic='edu-topic02-b', partition=2) 367
TopicPartition(topic='edu-topic02-b', partition=1) 500
TopicPartition(topic='edu-topic02-b', partition=1) 500
TopicPartition(topic='edu-topic02-b', partition=1) 500
TopicPartition(topic='edu-topic02-b', partition=1) 500
TopicPartition(topic='edu-topic02-b', partition=1) 500
TopicPartition(topic='edu-topic02-b', partition=1) 23
TopicPartition(topic='edu-topic02-b', partition=0) 477
TopicPartition(topic='edu-topic02-b', partition=0) 500
TopicPartition(topic='edu-topic02-b', partition=0) 500
TopicPartition(topic='edu-topic02-b', partition=0) 500
...
...
# args : topic, group-id, auto_offset_reset
$ python 45.consumer-poll.py edu-topic02 edu-topic02-cg latest
...
# 1) 10개 데이터를 보내 보자.
# args : topic, 
$ python 52.producer-10pub.py edu-topic02
...
# 1) consumer 수행
# args : topic, group-id
$ python 51.consumer-commit-60s.py edu-topic02 edu-topic02-cg
...
# 1) consumer current offset 초기화
# args : topic, group-id, auto_offset_reset
$ python 55.consumer-mannual-commit.py edu-topic02 edu-topic02-cg
