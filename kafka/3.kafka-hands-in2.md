...
- python  Consumer 실행한다.
  ...
  - ```sh
    ...
    $ python 11.consumer.py edu-topic01 edu-topic01-cg
...
- python producer 초당 1회 발송
  ...
  - ```sh
    ...
    # 자신의 topic 명을 아규먼트로 입력한다.
    # args : topic-name
    $ python 12.producer.py edu-topic01
    
    ```

    

- Grafana, kafkadrop 확인
...
# 10 회전송
$ python 21.producer-10pub.py edu-topic01-a
...
# topic partition 수정
$ kubectl -n kafka edit kafkatopic edu-topic01-a
...
# 확인
$ kubectl -n kafka get kafkatopic edu-topic01-a
NAME            CLUSTER      PARTITIONS   REPLICATION FACTOR   READY
...
$ python 22.producer-10pub_key.py edu-topic01-a key1
...
kafkadrop을 통해서 확인해 보면 특정 파티션 1개에만 보내지는 것을 확인 할 수 있다.
...
# key명을 아규먼트로 입력
$ python 22.producer-10pub_key.py edu-topic01-a key2
$ python 22.producer-10pub_key.py edu-topic01-a key3
...
kafkadrop을 통해서 확인해보면 partition 이 변경된 것을 확인할 수 있다.

토픽에 파티션을 추가하는 순간 key와 partition 의 일관성은 보장되지 않는다.
...
NAME            CLUSTER      PARTITIONS   REPLICATION FACTOR   READY
edu-topic01     my-cluster   3            2                    True
edu-topic01-a   my-cluster   3            1                    True
edu-topic01-b   my-cluster   3            3                    True
...
...
# 소스 확인
$ cat ./kafka/PythonSample/25.producer-spt-ack.py
...

# 3개의 ack 값을 아규먼트로 수행해 보자.
# args : range, ack
$ python ./kafka/PythonSample/25.producer-spt-ack.py 50000 0
$ python ./kafka/PythonSample/25.producer-spt-ack.py 50000 1
$ python ./kafka/PythonSample/25.producer-spt-ack.py 50000 all


# 로그를 확인해보자.
topicName[edu-topic01-b] range_cnt[50000] ack[0] Producing...
duration time : 4.446583032608032

topicName[edu-topic01-b] range_cnt[50000] ack[1] Producing...
duration time : 4.506533145904541

topicName[edu-topic01-b] range_cnt[50000] ack[all] Producing...
...
# 소스 확인
$ cat ./kafka/PythonSample/30.producer-spt-batch-linger.py
...

# batch size 를 변경해 보면서 수행해 보자.
# args : range, batch, linger
$ python ./kafka/PythonSample/30.producer-spt-batch-linger.py 50000 16384 3000
$ python ./kafka/PythonSample/30.producer-spt-batch-linger.py 50000 32768 3000
$ python ./kafka/PythonSample/30.producer-spt-batch-linger.py 50000 65536 3000


# 로그를 확인해보자.
topicName[edu-topic01-b] range_cnt[50000] batch_size[16384] linger_ms[3000] Producing...
duration time : 4.992011070251465

topicName[edu-topic01-b] range_cnt[50000] batch_size[32768] linger_ms[3000] Producing...
duration time : 4.646012544631958

topicName[edu-topic01-b] range_cnt[50000] batch_size[65536] linger_ms[3000] Producing...
...
# 소스 확인
$ cat ./kafka/PythonSample/41.consumer-offset-reset.py
...

# consumer 수행해 보자.
# args : group-id, auto_offset_reset
$ python ./kafka/PythonSample/41.consumer-offset-reset.py edu-topic01-a1-cg latest


# 로그를 확인해보자.
KafkaConsumer group_id[edu-topic01-a1-cg] auto_offset_reset[latest]  ...
topicName[edu-topic01-a] subscribed!
Consuming...
...
$ python ./kafka/PythonSample/41.consumer-offset-reset.py edu-topic01-a2-cg earliest


# 로그를 확인해보자.
KafkaConsumer group_id[edu-topic01-a2-cg] auto_offset_reset[earliest]  ...
topicName[edu-topic01-a] subscribed!
Consuming...
...
topic=edu-topic01-a partition=2 offset=36: key=b'{keyName}' value=b'{"eventName":"a","num":6,"title":"a", "writeId":"", "writeName": "", "writeDate":"" }'
topic=edu-topic01-a partition=2 offset=37: key=b'{keyName}' value=b'{"eventName":"a","num":7,"title":"a", "writeId":"", "writeName": "", "writeDate":"" }'
topic=edu-topic01-a partition=2 offset=38: key=b'{keyName}' value=b'{"eventName":"a","num":8,"title":"a", "writeId":"", "writeName": "", "writeDate":"" }'
topic=edu-topic01-a partition=2 offset=39: key=b'{keyName}' value=b'{"eventName":"a","num":9,"title":"a", "writeId":"", "writeName": "", "writeDate":"" }'
...


# 처음부터 읽기 때문에 모든 데이터를 다 가져온다.
```
...
$ python ./kafka/PythonSample/41.consumer-offset-reset.py edu-topic01-a1-cg earliest

KafkaConsumer group_id[edu-topic01-a1-cg] auto_offset_reset[earliest]  ...
topicName[edu-topic01-a] subscribed!
Consuming...
...
edu-topic01-a1-cg 은 이미 사용했으므로 current offset 이 이미 lastest 로 저장되었다.
...
# 소스 확인
$ cat ./kafka/PythonSample/45.consumer-poll.py
...

# 1) consumer current offset 초기화
# args : group-id, auto_offset_reset
$ python ./kafka/PythonSample/45.consumer-poll.py edu-topic01-b-cg latest
...
# 1) 대량 데이터 전송
$ python ./kafka/PythonSample/47.producer-spt-ack.py 50000
...
...
TopicPartition(topic='edu-topic01-b', partition=2) 500
TopicPartition(topic='edu-topic01-b', partition=2) 500
TopicPartition(topic='edu-topic01-b', partition=2) 367
TopicPartition(topic='edu-topic01-b', partition=1) 500
TopicPartition(topic='edu-topic01-b', partition=1) 500
TopicPartition(topic='edu-topic01-b', partition=1) 500
TopicPartition(topic='edu-topic01-b', partition=1) 500
TopicPartition(topic='edu-topic01-b', partition=1) 500
TopicPartition(topic='edu-topic01-b', partition=1) 23
TopicPartition(topic='edu-topic01-b', partition=0) 477
TopicPartition(topic='edu-topic01-b', partition=0) 500
TopicPartition(topic='edu-topic01-b', partition=0) 500
TopicPartition(topic='edu-topic01-b', partition=0) 500
...
```
...
# args : group-id, auto_offset_reset
$ python ./kafka/PythonSample/45.consumer-poll.py edu-topic01-cg latest
...
# 소스 확인
$ cat ./kafka/PythonSample/52.producer-10pub.py
...

# 1) 10개 데이터를 보내 보자.
$ python ./kafka/PythonSample/52.producer-10pub.py
...
# 소스 확인
$ cat ./kafka/PythonSample/51.consumer-commit-60s.py
...

# 1) consumer current offset 초기화
# args : group-id, auto_offset_reset
$ python ./kafka/PythonSample/51.consumer-commit-60s.py edu-topic01-b-cg latest
# 10 개 데이터 확인 
중지...  Ctrl+D
...
# 소스 확인
$ cat ./kafka/PythonSample/55.consumer-mannual-commit.py
...

# 1) consumer current offset 초기화
# args : group-id, auto_offset_reset
$ python ./kafka/PythonSample/55.consumer-mannual-commit.py edu-topic01-b-cg latest
