...
edu-topic01-a1-cg consumer group-id 내에 current offset 이 이미 저장되었기 때문에 earliest 라도 데이터를 읽지 않는다.
...
poll() 함수를 이용해서 consum 했을때 한번에 읽어들이는 건수를 확인해보자.
...
auto commit 상테에서는 commit 되기전에 consumer 가 중지후 다시 재처리되는 상황이 발생할 수 있다.
